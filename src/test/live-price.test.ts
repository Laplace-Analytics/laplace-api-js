...
import { BISTStockStreamData, BISTBidAskStreamData, LivePriceClient, OrderbookLiveData } from "../client/live-price";
...
describe("LivePrice", () => {
  ...
  describe("GetBidAskPriceForBIST", () => {
    it(
      "should receive BIST bid/ask price data",
      async () => {
        const symbols = ["AKBNK", "ISCTR"];
        let receivedData: BISTBidAskStreamData | null = null;
        let receivedError: Error | null = null;

        const lc = client.getBidAskPriceForBIST(symbols);
        activeConnections.push(lc);
        ...
        try {
          ...
          if (receivedData != null) {
            ...
            expect(tempReceivedData.bid).toBeLessThanOrEqual(tempReceivedData.ask);
            
            // Prices should be positive
            expect(tempReceivedData.bid).toBeGreaterThan(0);
            expect(tempReceivedData.ask).toBeGreaterThan(0);
          } else {
